...
The ARM environment consists of an Apple Silicon macOS host with a heterogeneous multi-core CPU. The virtualized environment is provisioned using UTM (QEMU-based), running an ARM64 Ubuntu instance allocated with 5 vCPUs. Testing relies on hardware-assisted virtualization without instruction translation, though utilizing a userspace NAT engine for networking.

\subsection{x86 Architecture Setup}
The x86 environment utilizes a dedicated x86\_64 machine with an NVMe SSD running Ubuntu Linux natively. For the virtual environment, Oracle VirtualBox hosts an Ubuntu VM using Intel VT-x hardware-assisted virtualization, dynamic VDI storage, and NAT networking.
...
Conversely, on x86 (VirtualBox), CPU virtualization exhibits near-native performance with a mere 1.86\% overhead for single-thread scenarios, showcasing a 6.8-fold efficiency advantage over the ARM/UTM implementation. Intel VT-x effectively delegates workloads to physical execution. Multi-threaded overhead slightly increases but remains mostly statistically insignificant (p=0.29) from bare metal, though experiencing marginally higher execution jitter.
...
Networking observed the most drastic virtualization taxation on both platforms. TCP throughput dropped by 48\%--54\% against local loopbacks globally, and round-trip time (RTT) average latency became 8.4 times higher (with minimum RTT showing a 10.2x increase) on ARM. A critical finding reveals that parallel TCP streaming actually degrades performance compared to single streams: increasing TCP streams from 1 to 8 caused total throughput to collapse by 45\%, outlining severe contention bottlenecks within single-threaded NAT engines. While throughput limits hit hard, packet reliability via UDP demonstrated that the hypervisor strictly shapes traffic without yielding packet loss. Both ARM and x86 face fundamentally similar virtual networking limitations.

\section{Comparative Analysis \& Lessons Learned}

While CPU operations are remarkably close to native speed (particularly due to mature extensions on x86), storage and network layers are penalized by abstraction constraints:
\begin{itemize}
    \item \textbf{Mature VT-x vs QEMU VM}: x86 hardware assistance handles instructions more efficiently.
    \item \textbf{Storage Stack}: Image formats like dynamically expanded `.vdi` dramatically inflate write latencies against raw disk translations. Virtualized systems targeting heavy DBMS should bypass these mechanisms via hardware passthrough.
    \item \textbf{NAT limitations}: The NAT networking paradigm in desktop hypervisors is extremely detrimental to highly parallel TCP workflows.
\end{itemize}

\section{Conclusion}
This study quantifies virtualization penalties comparatively. The x86 virtualization ecosystem delivers marginally better compute integration, effectively wiping out compute overhead, but suffers heavy disk emulation latency unless specifically optimized. The ARM UTM stack operates at a higher base compute tax but handles lightweight virtual networking stably. The results inform administrators to favor bare metal (or passthrough) for clustered SQL and parallel throughput clusters, while effectively treating VMs as raw compute nodes for compute-bound isolated services.
